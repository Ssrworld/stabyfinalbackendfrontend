...
exports.getFundTransferHistory = async (req, res) => { const userId = req.user.id; try { const transfers = await db('fund_transfers as ft').leftJoin('users as sender', 'ft.sender_id', 'sender.id').leftJoin('users as recipient', 'ft.recipient_id', 'recipient.id').select('ft.id', 'ft.amount', 'ft.source_wallet', 'ft.created_at', 'sender.email as sender_email', 'recipient.email as recipient_email').where('ft.sender_id', userId).orWhere('ft.recipient_id', userId).orderBy('ft.created_at', 'desc'); res.json(transfers.map(t => ({ id: t.id, date: t.created_at, amount: parseFloat(t.amount).toFixed(2), type: t.sender_email === req.user.email ? 'Sent' : 'Received', counterparty: t.sender_email === req.user.email ? t.recipient_email : t.sender_email, source: t.source_wallet }))); } catch (error) { console.error("FUND TRANSFER HISTORY ERROR:", error); res.status(500).json({ message: 'Error fetching transfer history.' }); } };
